...
import { submitCheckin } from "@/app/dashboard/client/actions";
    ...
    return (
      <div className="bg-gray-900/50 p-6 rounded-2xl border border-gray-700 text-center">
        ...
        <p className="text-sm text-gray-400 mt-2">
          Great work this week. Your next check-in is due in a few days.
        </p>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="bg-amber-500/10 p-6 rounded-2xl border border-amber-500/30 text-center">
        ...
        <Button onClick={() => setIsOpen(true)} className="mt-4" size="sm">
          Start Check-in
        </Button>
      </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-gray-900 p-8 rounded-2xl border border-gray-700 max-w-2xl w-full">
            <h4 className="text-xl font-bold text-amber-400 mb-6">
              Weekly Check-in
            </h4>
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex justify-between mt-2">
                  {[1, 2, 3, 4, 5].map((val) => (
                    <button
                      ...
                      key={val}
                      onClick={() =>
                        setFormState({ ...formState, energyLevel: val })
                      }
                      className={`w-10 h-10 rounded-full transition-all ${formState.energyLevel === val ? "bg-amber-500 text-black" : "bg-gray-700 text-white"}`}
                    >
                      {val}
                    </button>
                  ))}
              ...
              <div>
                <label className="text-sm font-semibold text-white">
                  Rate your motivation this week (1-5)
                </label>
                <div className="flex justify-between mt-2">
                  {[1, 2, 3, 4, 5].map((val) => (
                    <button
                      ...
                      key={val}
                      onClick={() =>
                        setFormState({ ...formState, motivationLevel: val })
                      }
                      className={`w-10 h-10 rounded-full transition-all ${formState.motivationLevel === val ? "bg-amber-500 text-black" : "bg-gray-700 text-white"}`}
                    >
                      {val}
                    </button>
                  ))}
              ...
              <div>
                ...
                <textarea
                  value={formState.winsForTheWeek}
                  onChange={(e) =>
                    setFormState({
                      ...formState,
                      winsForTheWeek: e.target.value,
                    })
                  }
                  rows={3}
                  className="mt-1 block w-full rounded-md bg-white/5 py-2 px-3 text-white ring-1 ring-white/10"
