...
import { logWorkout } from '@/src/app/dashboard/client/actions';
...
export function WorkoutPlanDisplay({ plan }: { plan: WorkoutPlan }) {
    ...
    const handleSetInputChange = (exIndex: number, setIndex: number, field: keyof SetLog, value: string) => {
                ...
                {plan.content.days.map((day, dayIndex) => (
                    <div key={day.id || dayIndex} className="bg-black/20 p-4 rounded-lg">
                        <div className="flex justify-between items-center mb-3">
                            <h4 className="font-semibold text-lg text-white">{day.dayTitle || `Day ${dayIndex + 1}`}</h4>
                            <Button variant="outline" size="sm" onClick={() => handleOpenLogger(day)}>Log This Day</Button>
                        </div>
            ...
            {loggingDay && (
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
                    <div className="bg-gray-900 p-8 rounded-2xl border border-gray-700 max-w-2xl w-full max-h-[90vh] flex flex-col">
                        <h4 className="text-xl font-bold text-amber-400 mb-4 flex-shrink-0">Log: {loggingDay.dayTitle}</h4>
                        <div className="space-y-6 overflow-y-auto flex-grow pr-2">
                            {loggingDay.exercises.map((exercise, exIndex) => (
                                <div key={exercise.id} className="p-4 bg-black/20 rounded-lg">
                                    <p className="font-semibold text-white mb-4">{exercise.name}</p>
                                    <div className="space-y-3">
                                        ...
                                        {formState[exIndex]?.sets.map((set, setIndex) => (
                                            <div key={setIndex} className="grid grid-cols-3 gap-3 items-center">
                                                <span className="text-sm text-gray-400 font-medium">Set {setIndex + 1}</span>
                                                <input
                                                    type="text"
                                                    value={set.reps}
                                    ...
                                    <div className="mt-4">
                                        <label className="text-xs font-semibold text-gray-400">Notes (optional)</label>
                                        <textarea value={formState[exIndex]?.notes || ''} onChange={e => handleNotesChange(exIndex, e.target.value)} placeholder="How did it feel?" rows={2} className="mt-1 block w-full rounded-md bg-white/5 py-2 px-3 text-white ring-1 ring-white/10"></textarea>
                                    </div>
                                </div>
                            ))}
                        </div>
                        <div className="flex justify-end gap-4 pt-6 border-t border-gray-700 flex-shrink-0">
                            {message && <p className="text-center text-sm mr-auto self-center">{message}</p>}
                            <Button type="button" variant="ghost" onClick={handleCloseLogger} disabled={isSubmitting}>Cancel</Button>
                            <Button type="button" onClick={handleSubmitLog} disabled={isSubmitting}>
