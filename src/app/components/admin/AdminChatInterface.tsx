    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // Only react if the message involves the admin
          if (sender_id === adminId || receiver_id === adminId) {
            // Check if this message belongs to the CURRENTLY OPEN chat
            const isFromSelectedClient = sender_id === selectedClientId;
            const isToSelectedClient = receiver_id === selectedClientId;

            if (isFromSelectedClient || isToSelectedClient) {
              // Avoid adding the optimistic message twice (check ID)
              setMessages((current) => {
                if (current.find((m) => m.id === payload.new.id))
                  return current;
                return [...current, payload.new as Message];
              });
            } else if (sender_id !== adminId) {
              // Notification logic for other clients
              setNotifications((current) => ({
                ...current,
                [sender_id]: (current[sender_id] || 0) + 1,
              }));
            }
          }
        },
            ...
            <div className="flex-grow p-6 overflow-y-auto space-y-4">
              {isLoading ? (
                ...
                messages.map((msg) => (
                  <div
                    key={msg.id}
                    className={`flex items-end gap-2 ${msg.sender_id === adminId ? "justify-end" : "justify-start"}`}
                  >
                    <div
                      className={`max-w-xs lg:max-w-md p-3 rounded-lg ${msg.sender_id === adminId ? "bg-amber-500 text-black" : "bg-gray-700 text-white"}`}
                    >
