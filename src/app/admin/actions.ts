...
export async function createWorkoutPlan(planData: PlanData) {
  ...
  if (!planData.clientId || !planData.planTitle || planData.days.length === 0) {
    return {
      success: false,
      message: "Client, title, and at least one day are required.",
    };
  }
  ...
  if (error) {
    ...
    return {
      success: false,
      message: "Database error: Could not create plan.",
    };
  }
...
export async function approveMeeting(meetingId: string) {
  ...
  if (!refreshToken) {
    console.error(
      "Configuration Error: Admin's Google account is not connected. No refresh token found.",
    );
  ...
  if (!meeting || !meeting.profiles?.email) {
    console.error(
      `Data Error: Could not find meeting with ID ${meetingId} or client email is missing.`,
    );
    return;
  }
  ...
  try {
    ...
    console.error(
      "Fatal Error: Could not create Google Calendar event or update database.",
      error,
    );
...
export async function getChartDataForClient(
  clientId: string,
  exerciseName: string,
) {
...
export async function uploadMealPlan(formData: FormData) {
  ...
  const {
    data: { publicUrl },
  } = supabase.storage.from("meal-plans").getPublicUrl(filePath);
