...
import { WorkoutPlanDisplay } from '@/src/app/components/client/WorkoutPlanDisplay';
import { WeeklyCheckin } from '@/src/app/components/client/WeeklyCheckin';
...
export default async function ClientDashboard() {
    ...
    const [
        ...
        supabase.from('profiles').select('full_name, meal_plan_url').eq('id', userId).single(),
        supabase.from('workout_plans').select('id, title, content').eq('client_id', userId).order('created_at', { ascending: false }).limit(1).single(),
        supabase.from('meetings').select('id, requested_time, status, meeting_link').eq('client_id', userId).order('requested_time', { ascending: false }),
        ...
        supabase.from('weekly_checkins').select('created_at').eq('client_id', userId).order('created_at', { ascending: false }).limit(1).single(),
    ]);
        ...
        <div className="bg-black text-white min-h-screen">
            ...
            <main className="w-full max-w-7xl mx-auto p-6 lg:px-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    {workoutPlan ? (
                        <WorkoutPlanDisplay plan={workoutPlan as any} />
                    ) : (
                    ...
                    {profile?.meal_plan_url && (
                        <div className="bg-gray-900/50 p-8 rounded-2xl border border-gray-700">
                            <h3 className="text-2xl font-bold text-amber-400 mb-4">Your Meal Plan</h3>
                            <p className="text-gray-300 mb-6">Your personalized meal plan is ready. Click the button below to view and download it.</p>
